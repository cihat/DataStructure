...
public class Stack {
  int kapasite;
  int[] s; // ! Dizi şeklinde tutuyoruz.
  int ust; // ! Üst adında bir değişken tutup bununla sınırlarımızı belirleyip/kontrol
           // ! edeceğiz.

  public Stack(int kapasite) {
    this.kapasite = kapasite;
    s = new int[kapasite]; // * dışarıdan girilen boyutla dizimizi oluşturduk.
    ust = -1; // * dışarıdan girilen boyutla dizimizi oluşturduk.
  }
  ...
  // * Eğer tüm ekleme ve çıkarma işlemlerinin sonunda üst değişkenimiz hala -1'i
  // * gösteriyor ise boştur.
  boolean bosmu() {
    return ust == 1;
  }

  // * sınırımızı bir arttırıp gelen elemanı dizinin indisine ekliyoruz.(Eğer
  // stack
  // * dolu değilse..)
  void push(int x) {
    if (!dolumu()) {
  ...
  // * Eğer stack dolu ise(boş değilse) sınırı azaltıp en tepedeki değeri bize
  // * göstermesini söylüyoruz.
  int pop() {
    if (!bosmu()) {
      ust = ust - 1;
