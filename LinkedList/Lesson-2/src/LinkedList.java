...
public class LinkedList {
  ...
  public void sonaEkle(Eleman yeni) {
    //Eğer hiç eleman yoksa gelen eleman hem baş hem de sonu gostermeli.basaEkle fonk. da ki gibi.
    if (yeni == null) {
      bas = yeni;
      son = yeni;
    } else {
      //Eğer daha onceden eleman varsa yeni gelen eleman sonu göstermeli ve artık son o oluyor.
      son.ileri = yeni;
      son = yeni;
    }
  }

  //arayaEkleme() method ile birlikte liste içinde aradığımız elemana kadar gidip o elemanı bulduktan sonra hemen arkasına eklemek
  ...
  //baş bir sonraki elemanı gösterdiğinde o elemanı artık kaybetmiş(silmiş) oluyourz.
  ...
  //Aradığımız elemanı bulmdığumuzda öncesinde bulunan elemanla aradığımız elemanın bir sonraki elemanı arasında
  ...
  void siraliEkleme(Eleman yeni) {
    if (bas == null) {
      ...
      //Gezinme işleminde kullandığımız 'yedek' değişkeni boş olmadığı ve yeni gelen sayimiz bir önceki elemandan büyük
      ...
      //Eğer daha önceden bir eleman varsa yeni gelen elemanin ilerisi artık bir önceki elemanın ilerisini gösterecek
      //bir önceki elemaın ilerisinde artık yeni eleman olcağından dolayı bir nevi 'araya ekleme' olacaktır.
      else {
  ...
  //Burada ilk emelandan başlayıp son eleman boş olana kadar gezinip tek tek hepsini bir String'e atıyoruz.
  String listele() {
    Eleman yedek = bas;
    String liste = "";
    while (yedek != null) {
